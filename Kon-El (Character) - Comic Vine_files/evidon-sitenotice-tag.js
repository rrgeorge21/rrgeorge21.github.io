(function() {
    if (!Object.keys) {
        Object.keys = (function() {
            var au = Object.prototype.hasOwnProperty,
                at = !{ toString: null }.propertyIsEnumerable("toString"),
                aq = [
                    "toString",
                    "toLocaleString",
                    "valueOf",
    ...
    var o = crownpeakCdnHost + crownpeakCdnPath,
        al = o + "/sitenotice/",
        r = al + window.evidon.id,
        b = 1,
        a = 2,
        x = 1,
        v = 2,
    ...
    var f = function() {
        ...
        function aw() {
            ...
            if (aM) {
                ...
                aF.className = k;
                var aO =
                    "border:0;display:inline;vertical-align:middle;margin-right:10px;";
        ...
        function at() {
            ...
            if (ax.length === 0) {
                console.log(
                    "Evidon -- " +
                    T +
                    " not found on page, cant display the consent link."
                );
            }
    ...
    var am = function() {
        ...
        if (aA) {
            ...
            if (az !== -1) {
                aB = aB.substr(0, az + 4);
                o = aB;
                al = o + crownpeakCdnPath + "/sitenotice/";
                r = al + window.evidon.id;
            }
    ...
    am.prototype._getConsentCookieDomainPath = function() {
        ...
        if (aq === 2) {
            ...
            if (!ar) {
                console.error(
                    "Attempted to set a cookie without a valid settings object"
                );
                return;
            }
            var at = ar.indexOf("/");
    ...
    am.prototype._getConsentCookieDomain = function() {
        ...
        if (aq === 1) {
            return (
                "." + this._getRootDomain(this.domain) + "." + this._getTLD(this.domain)
            );
        }
        return null;
    };
    ...
    am.prototype._setConsentCookie = function(ax, aq, ay, av) {
        ...
        if (
            this._writeLocalStorage(t + this.activeSettings.id, JSON.stringify(ar))
        ) {
    ...
    am.prototype._updateConsentData = function(aw, au) {
        ...
        if (
            !aq.hasOwnProperty(aw) ||
            !aq[aw].hasOwnProperty(this.activeCountryId)
        ) {
            return;
        }
        aq[aw][this.activeCountryId] = au;
        if (
            this._writeLocalStorage(t + this.activeSettings.id, JSON.stringify(aq))
        ) {
    ...
    am.prototype._triggerTagManagerEvent = function() {
        ...
        try {
            if (window.utag) {
                window.utag.link({ event_name: ao });
            }
        } catch (ar) {}
                ...
                if (this.translations.hasOwnProperty(this.languageRoot)) {
                    ...
                    if (ar) {
                        ...
                        if (av.hasOwnProperty("items")) {
                            this.activeTranslations = this._setTranslationValues(
                                av[ar],
                                av.items
                            );
                        } else {
    ...
    am.prototype.getPrivacyPolicyLink = function() {
        ...
        if (
            aq &&
            ar.privacyLinks &&
            ar.privacyLinks.hasOwnProperty(aq.privacyLinkId)
        ) {
    ...
    am.prototype.showConsentTool = function(aw) {
        ...
        var ax =
            at == K || (at == L && window.location.protocol.indexOf("https") === -1);
        if (ax) {
    ...
    am.prototype.isMobile = function() {
        ...
        return (
            /ip(hone|od)|(android).+mobile|opera m(ob|in)i/i.test(aq) ||
            /Android/.test(aq) ||
            /iPhone/.test(aq) ||
            /iPad/.test(aq)
        );
    ...
    am.prototype.showNotice = function() {
        ...
        var ar = "";
        if (
            aw.hasOwnProperty("consentTemplate") &&
            window.evidon.consentTemplates
        ) {
            ...
            this.consentRequired =
                au.consentRequired === undefined ? au.duration > 0 : au.consentRequired;
            ...
            this.closeConsentEnabled =
                au.closeConsentEnabled === undefined ? true : au.closeConsentEnabled;
            ...
            this.consentRequired =
                aw.consentRequired === undefined ? aw.duration > 0 : aw.consentRequired;
            ...
            this.closeConsentEnabled =
                aw.closeConsentEnabled === undefined ? true : aw.closeConsentEnabled;
            ar = aw.consentactions;
        }
        this.dataRightsType =
            aw.dataRightsType === undefined ? 0 : aw.dataRightsType;
    ...
    am.prototype.getActiveVendors = function() {
        if (
            !this.activeSettings ||
            !this.activeSettings.hasOwnProperty("vendors")
        ) {
            return;
        }
    ...
    am.prototype.getPreferencesDialogV2Enabled = function() {
        ...
        return this.isMobile()
            ? aq.mobileEnablePreferencesDialogV2
            : aq.enablePreferencesDialogV2;
    };
    ...
    var A = function() {
        ...
        this.subscribe = function(ar, aq, at) {
            ...
            this.subscriptions[ar].push({ callback: aq, limit: at });
        };
